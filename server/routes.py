...
def calculate_popularity(song):
    ...
    if song.popularity_score == 0.0:
        #  formula for popularity calculation
...
def create_song():
    ...
    db.session.add(song)
    db.session.commit()
    calculate_popularity(song)  # Calculate and store popularity
...
def update_song(song_id):
    ...
    db.session.commit()
    calculate_popularity(song)  # Recalculate popularity after updating metrics
